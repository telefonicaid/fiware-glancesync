...
def _update_metadata_remote(region, image):
   ...
   # compose cmd line
   arguments = [ 'glance' , 'update', image['Id'] , 'disk_format='+image['Disk format'],
        'name='+ image['Name'] , 'is_public='+image['Public'] , 'protected='+image['Protected'],
...
def _upload_image_remote(region, image, replace_uuid=None, rename_uuid=None):
   ...
   if replace_uuid or rename_uuid:
      ...
      if saved_type: del(image['_type'])
      saved_nid = image.get('_nid',None)
      if saved_nid: del(image['_nid'])
   ...
   props = list( x[1:] + '=' + image[x] for x in image if x.startswith('_'))
   # compose cmdline
   arguments = [ 'glance' , 'add', '--silent-upload' , 'disk_format='+image['Disk format'],
        'name='+ image['Name'] , 'is_public='+image['Public'] , 'protected='+image['Protected'],
        'container_format=' + image['Container format'] ]
   arguments.extend(props)
   # run command
   ...
   p.wait()
   if p.returncode != 0:
      print 'Upload of ' + image['Name'] + " to region " + region + ' Failed.'
      print result
      sys.exit(-1)
   ...
   if rename_uuid or replace_uuid:
      newimage=dict(image)
      newimage['Id']=newuuid
      if saved_nid: newimage['_nid']=saved_nid
      if saved_type: newimage['_type']=saved_type
...
def _getimagelist(region, region_uri):
  """returns a list of images from the glance of the specified region
   ...
   Each imagen is a dictionary indexed by name. Extra properties are coded as _<name>
   the other labels are the returned by glance details. List is completed with checksum.
  """
  os.environ['OS_REGION_NAME']=region
  images=list()
  image=None
  ...
  for line in outputcmd:
     if line[0]=='=':
       if image!=None and image['Name']!='None':
...
def _prefix(image, comparewith):
  name = image['Name']
  if not name in comparewith: return '+'
  if image['Status'] != 'active': return '$'
  ...
  if image_master_region.get('Public',None) != image.get('Public',None):
      if image_master_region.get('Public',None) == 'No':
          ...
          return '#'
  if image_master_region.get('_nid',None) != image.get('_nid',None): return '#' 
  if image_master_region.get('_type',None) != image.get('_type',None): return '#' 
  if image_master_region.get('_sdc_aware',None) != image.get('_sdc_aware',None): return '#' 
  return ''

def _printimages(imagesregion, comparewith=None):
  images = list (image for image in imagesregion if image['Public']=='Yes' and ( '_nid' in image and '_type' in image))
  ...
  print "---"
  images = list (image for image in imagesregion if image['Public']=='Yes' and (not '_nid' in image and  '_type' in image))
  images.sort(key=lambda image: image['_type']+image['Name'])
  for image in images:
     line = _convert2csv(image,('Name','_type','_nid') ) 
     if line != None:
       if comparewith!=None:
          ...
          print line
  print "---"
  images = list (image for image in imagesregion if image['Public']=='Yes' and ( '_nid' in image and not '_type' in image))
...
def _get_regions_uris(region_list):
   ...
   for region in region_list:
      regionsuris[region]=kc.service_catalog.url_for('region',region,'image')
   return regionsuris
...
def _get_master_region_dict(master_region, master_region_uri): 
  ...
  for image in images:
     if '_kernel_id' in image:
         image['_kernel_id'] = master_region_dictimagesbyid[image['_kernel_id']]['Name'] 
     if '_ramdisk_id' in image:
         image['_ramdisk_id'] = master_region_dictimagesbyid[image['_ramdisk_id']]['Name'] 
