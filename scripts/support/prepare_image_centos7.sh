...
VM_ID=$(./launch_image.py centos7 centos7.orig m1.small)
sleep 180
$unsecurescp support_pubkeys/defaultgpg.pub support_pubkeys/defaultssh.pub activate_support_account.py resources/systemd $USER@10.95.55.170:
cat <<EOF | $unsecuressh $USER@10.95.55.170 -t -t
sudo yum -y update
sudo yum -y install yum-cron wget
sudo mv activate_support_account.py /sbin
sudo mv systemd /usr/lib/systemd/system/active-support.service 
sudo chown root:root /usr/lib/systemd/system/active-support.service 
sudo chmod 755 /usr/lib/systemd/system/active-support.service 
sudo systemctl enable active-support
sudo install -d /etc/fiware-support
sudo mv defaultgpg.pub defaultssh.pub /etc/fiware-support/
exit
EOF
nova image-create ${VM_ID} ${IMAGE}snapshot --poll
nova delete ${VM_ID}
